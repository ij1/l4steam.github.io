...
declare -A CCA
for j in 1 2; do
    for i in $(seq $j 2 $HOST_PAIRS); do
        ECN[s$i]=$(eval echo '$'C${j}_ECN)
        ECN[c$i]=$(eval echo '$'C${j}_ECN)
        CCA[c$i]=$(eval echo '$'C${j}_CCA)
        CCA[s$i]=$(eval echo '$'C${j}_CCA)
    done
done
...
DO_TCP_S1_C1=${DO_TCP_S1_C1:-}
DO_TCP_S2_C2=${DO_TCP_S2_C2:-}
DO_UDP=${DO_UDP:-}
DO_UDP_ECT0=${DO_UDP_ECT0:-}
DO_UDP_ECT1=${DO_UDP_ECT1:-}
...
function run_test()
{
    ...
    iperf_servers s1 s2
    iperf_server s3 "_udp_${DO_UDP}"
    iperf_server s4 "_udp-ect0_${DO_UDP_ECT0}"
    iperf_server s5 "_udp-ect1_${DO_UDP_ECT1}"

    echo "Running tests for ${TIME}sec"
    if [ "x${DO_TCP_S1_C1}" != "x" ]; then
        iperf_client c1 s1 "" &
        pid_iperf=$!
    fi
    if [ "x${DO_TCP_S2_C2}" != "x" ]; then
        iperf_client c2 s2 "" &
        pid_iperf=$!
    fi
    if [ "x${DO_UDP}" != "x" ]; then
        iperf_client c3 s3 "_udp_$DO_UDP" -u -b $DO_UDP --length 1200 &
        pid_iperf=$!
    fi
    if [ "x${DO_UDP_ECT0}" != "x" ]; then
        iperf_client c4 s4 "_udp-ect0_$DO_UDP_ECT0" -u -S0 -b $DO_UDP_ECT0 --length 1200 &
        pid_iperf=$!
    fi
    if [ "x${DO_UDP_ECT1}" != "x" ]; then
        iperf_client c5 s5 "_udp-ect1_$DO_UDP_ECT1" -u -S1 -b $DO_UDP_ECT1 --length 1200 &
        pid_iperf=$!
    fi
    wait $pid_iperf
...
function usage()
{
    ...
    echo "      C1_CCA [$C1_CCA] -- The net.ipv4.tcp_congestion_control sysctl for the cX-sX pairs (where X is odd)"
    echo "      C2_CCA [$C2_CCA] -- The net.ipv4.tcp_congestion_control sysctl for the cX-sX pair (where X is even)"
    echo "      C1_ECN [$C1_ECN] -- The net.ipv4.tcp_ecn sysctl for the cX-sX pair (where X is odd)"
    echo "      C2_ECN [$C2_ECN] -- The net.ipv4.tcp_ecn sysctl for the cX-sX pair (where X is even)"
    echo ""
    ...
    echo ""
    echo "  You can control which background transfert will be executed through:"
    echo "      DO_TCP_S1_C1 [${DO_TCP_S1_C1}] -- Run a TCP flow for the s1-c1 pair "
    echo "      DO_TCP_S2_C2 [${DO_TCP_S2_C2}] -- Run a TCP flow for the s2-c2 pair"
    echo "      DO_UDP [${DO_UDP}] -- Run an UDP flow for the s3-c3 pair of the given bandwidth"
    echo "      DO_UDP_ECT0 [${DO_UDP_ECT0}] -- Run an UDP flow for the s4-c4 pair of the given bandwidth and mark it ECT(0)"
    echo "      DO_UDP_ECT1 [${DO_UDP_ECT1}] -- Run an UDP flow for the s5-c5 pair of the given bandwidth and mark it ECT(1)"
    echo ""

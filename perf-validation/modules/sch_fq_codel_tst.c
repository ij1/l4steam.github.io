...
static inline struct testbed_metrics* fq_codel_metrics(struct Qdisc *sch)
{
				  ...
				  struct sk_buff **to_free)
{
	...
	do {
		...
		testbed_inc_drop_count(fq_codel_metrics(sch), skb, 0);
...
begin:
	...
	testbed_add_metrics(skb, fq_codel_metrics(sch),
			    (ktime_get_ns() >> 10) - codel_get_enqueue_time(skb), 0);
	return skb;
}
...
static void fq_codel_reset(struct Qdisc *sch)
{
	...
	q->memory_usage = 0;
	testbed_metrics_init(fq_codel_metrics(sch));
}
